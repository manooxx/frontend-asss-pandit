...
const CategoriesPage = () => {
  ...
  return (
    <div className="p-6 w-full flex flex-col justify-center items-center py-16 z-30">
     ...
     <div className='w-[550px] fixed right-[-250px] top-32 z-10'>
      ...
      <h1 className="text-4xl font-semibold mb-4">Select a Category</h1>
      <ul className='grid grid-cols-2 gap-4 mb-7 z-30'>
        {categories.map((category) => (
          <li
            key={category.id}
            onClick={() => setSelectedCategory(category.id)}
            className={`mb-2 p-2 z-30 shadow-md hover:shadow-xl duration-300 border rounded-lg  w-full flex cursor-pointer ${selectedCategory === category.id ? 'border-1 border-red-600' : ''}`}
          >
